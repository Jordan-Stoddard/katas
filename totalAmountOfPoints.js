// Our football team finished the championship. The result of each match look like "x:y". Results of all matches are recorded in the array.

// For example: ["3:1", "2:2", "0:1", ...]

// Write a function that takes such list and counts the points of our team in the championship. Rules for counting points for each match:

// if x>y - 3 points
// if x<y - 0 point
// if x=y - 1 point
// Notes:

// there are 10 matches in the championship
// 0 <= x <= 4
// 0 <= y <= 4

const seasonScore = ['1:0','2:0','3:0','4:0','2:1','3:1','4:1','3:2','4:2','4:3']; // === 30

function points(games) {
  const scores = games.map(gameScore => {
    let x = parseInt(gameScore.charAt(0));
    let y = parseInt(gameScore.charAt(2));
    
    if (x > y) {
      return 3;
    }
    if (x < y) {
      return 0;
    }
    if ((x === y)) {
      return 1;
    }
  });
  return scores.reduce((current, accumulator) => current + accumulator, 0);
}

console.log(points(seasonScore));
